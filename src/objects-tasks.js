...
function mergeObjects(objects) {
  ...
  return copy.reduce((acc, current) => {
...
function makeWord(lettersObject) {
  let word = '';
  const length = Math.max(...Object.values(lettersObject).flat());
  for (let i = 0; i <= length; i += 1) {
    word += findLetter(lettersObject, i);
  }
  return word;
}
...
function fromJSON(proto, json) {
  const arrOfValues = Object.values(JSON.parse(json));
  return new proto.constructor(...arrOfValues);
}
...
function sortCitiesArray(arr) {
  const copyArr = [...arr];
  copyArr.sort((a, b) => {
    const sortCountry = a.country.localeCompare(b.country);
    if (sortCountry === 0) {
